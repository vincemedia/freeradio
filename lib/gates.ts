 ...
 *
 * There is no admission verdict here any more. Holdings belong to a wallet and
 * this app does not read wallets, so a room's terms are described rather than
 * judged: the badge says what the door asks for and stops short of claiming
 * you would get through it. `validateGates` stays, because a half-configured
 * gate is still a broken one.
 */
